...
%   whatsoever.
%----------------------------------------------------------------------------
function [daughter,fourier_factor,coi,dofmin,Psi0] = ...
	wave_bases(mother,k,scale,param);
...
if (strcmp(mother,'MORLET'))  %-----------------------------------  Morlet
	if (param == -1), param = 6.;, end %param 6.
	k0 = param;
...
elseif (strcmp(mother,'PAUL'))  %--------------------------------  Paul
	...
	dofmin = 2;
    Psi0 = 2^m*i^m*prod(1:m)/sqrt(pi*prod(1:2*m));
elseif (strcmp(mother,'DOG'))  %--------------------------------  DOG
	...
	coi = fourier_factor/sqrt(2);
	dofmin = 1;
    Psi0 = i^(m+1)/gamma(m+0.5)*(-1); %for m = 2
